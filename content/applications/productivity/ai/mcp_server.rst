...
In the |MCP| workflow, an Odoo database acts as a remote |MCP| *server* that communicates back and
forth with the |MCP| *client* (the user's external |AI| agent) to execute :doc:`tools
<server-actions>` such as reading data or editing records in the database. This process happens
internally, invisible to the user, who **only** interacts with the |MCP| client by prompting it with
specific tasks.
...
#. The client requests the server to invoke the correct tools.
#. After executing the tool, the server returns the results back to the client.
...
#. Connect to the Odoo |MCP| server via the client.
#. Expose server actions to the client from Odoo.
...
On the *Change my Preferences* pop-up window, click the *Security* tab. Then, select :guilabel:`Add
...
.. tabs::
   ...
   .. tab:: Claude
      ...
      **Via Claude Desktop**
      ...
      `claude_desktop_config.json` and paste the following JSON snippet between the outer-most
      ellipses:
   ...
   .. tab:: Antigravity
      ...
      *Installed MCP Servers* section lists all configured |MCP| servers along with their statuses.
      ...
      `mcp_config.json` and paste the following JSON snippet between the outer-most ellipses:
      ...
      **Via Antigravity CLI**
      ...
      The *Antigravity CLI* configuration follows the same process as that of *Antigravity 2.0*.
   ...
   .. tab:: Codex
      ...
      Configuring an Odoo |MCP| server in *Codex* can be done either via the *Codex CLI* or directly
      editing the configuration file.

      **Via config file**

      Open the `config.toml` file, located in your root directory (`~/.codex/config.toml`). Then,
      ...
      .. note::
         While the `config.toml` is located globally in your system's root directory, it can also be
...
.. seealso::
   See the following documentation for more information about configuring your respective client:
...
The remaining tools are hidden from the client and must be manually exposed in order to be invoked.
...
.. list-table::
   ...
   * - `AI Tool: Get Models`
     ...
     - True
   * - `AI Tool: Get Fields`
     - Lists the searchable/usable fields on a given model, including type and relations.
     - True
   * - `AI Tool: Get Menus`
   ...
   * - `AI Tool: Get Menu Details`
     - Returns detailed information for specific menu IDs, such as action, domain, default
       filters/groupings, and searchable fields.
     - True
   * - `AI Tool: Search`
...
.. list-table::
   ...
   * - `AI Tool: Open Menu List`
     - Redirects the UI to a menu's List view, optionally filtered/grouped.
     - True
   * - `AI Tool: Open Menu Kanban`
     - Redirects the UI to a menu's Kanban view, optionally filtered/grouped.
     - True
   ...
   * - `AI Tool: Open Menu Pivot`
     - Redirects the UI to a menu's Pivot view with row/column groupings and measures.
     - True
   ...
   * - `AI Tool: Compute Report Measures`
     - Returns the measures available for aggregation on a given model/action. This tool is meant to
       be called before opening a pivot or graph view.
     - True
   ...
   * - `AI Tool: Adjust Search`
     - Adjusts the search filters, groupbys, active measures, or view type of the current view.
     - True
...
.. list-table::
   ...
   * - `AI Tool: Web Search`
     ...
     - True
   * - `AI Tool: Load Skills`
     - Loads detailed instructions/tools for specific registered AI skills by ID.
     - True
