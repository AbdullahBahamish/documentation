...
- :ref:`Existing Odoo users with products <shopify/manage/existing-odoo-users>`
...
Existing Odoo users with products
---------------------------------

Depending on the scenario, there are two methods to add Shopify products into an existing product
catalog in the **Sales** app: :ref:`adding a new Shopify product to the Sales app
<shopify/manage/add-new-products>` and :ref:`syncing existing products in Shopify and the Sales app
<shopify/manage/sync-existing-products>`.

.. _shopify/manage/add-new-products:

Adding a new Shopify product to the Sales app
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
The :guilabel:`Create Products` setting assists with product creation and mapping when using
:icon:`fa-refresh` :guilabel:`Fetch Orders` or :icon:`fa-refresh` :guilabel:`Fetch Products` to pull
Shopify data. Refer to the :ref:`shopify/setup/administrative-settings` section of the *Shopify
Connector configuration* page for the setting description.

To enable the setting, go to :menuselection:`Sales app --> Configuration --> Shopify: Accounts` and
select the desired Shopify account form. Click the :guilabel:`Configuration` tab, and in the
*Products* section, select the :guilabel:`Create Products` checkbox.

Then, in the *Operations* tab, either click the :icon:`fa-refresh` :guilabel:`Fetch Orders` or
:icon:`fa-refresh` :guilabel:`Fetch Products` action to sync products. Both actions follow the same
logic when the :guilabel:`Create Products` setting is enabled, the difference is in which products
are fetched.

- :icon:`fa-refresh` :guilabel:`Fetch Products` pulls all products in the Shopify store that are
  created or updated after the specified date.
- :icon:`fa-refresh` :guilabel:`Fetch Orders` pulls orders created or updated after the specified
  date. Only products that are in the pulled sales orders are updated or created.

.. note::
   After selecting a :guilabel:`Created/Updated after` date for the :icon:`fa-refresh`
   :guilabel:`Fetch Products` action, a warning pop-up displays. Users can still continue the action
...
intervals (every 10 minutes). Go to :menuselection:`Sales app --> Configuration --> Shopify:
Accounts`, select the desired Shopify account form, and click the *Operations* tab.
...
.. example::
   ...
   Michelle Admin noticed an the latest order from her Shopify store failed to import in to the
   **Sales** app. In the Odoo, she goes to :menuselection:`Sales app --> Configuration -->
   ...
   A *Recover Order* pop-up window displays with a blank :guilabel:`Ecommerce Order Reference`
...
When :guilabel:`Update Inventory` is enabled, Odoo pushes stock to Shopify after orders are pulled
...
To push stock manually, go to :menuselection:`Sales app --> Configuration --> Shopify: Accounts` to
select the desired Shopify account form. Click the :icon:`fa-refresh` :guilabel:`Push Inventory`
button in the *Operations* tab.
...
To pull stock manually, go to :menuselection:`Sales app --> Configuration --> Shopify: Accounts` to
select the desired Shopify account form. Click the :icon:`fa-refresh` :guilabel:`Fetch Inventory`
button in the *Operations* tab.
