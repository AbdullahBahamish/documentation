...
.. important::
   User must be in :ref:`developer mode <developer-mode/activation>` to configure the Shopify
   account form.
...
- :guilabel:`Create Products`: Controls what happens when a Shopify product is pulled and no Odoo
  ...
  When enabled, if no match is found, a new Odoo product is created from the Shopify data. Once a
...
Switching to the Odoo browser tab, paste these into the :guilabel:`Shopify Client Id` and
:guilabel:`Shopify Client Secret` in the *Credentials* tab.
...
store locations then the inventory for those Shopify account forms are synced with the internal Odoo
location.
