...
- Navigate to the main *Job Positions* dashboard by navigating to :menuselection:`Recruitment app
  --> Applications --> By Job Positions`. Next, click on the desired job position card, then click
  on the individual applicant card from the *Applications* page.
...
Select a refusal reason to :ref:`send a refusal email
<recruitment/refuse_applicant/send-refusal-email>`.
...
the desired email template from the list to be used when the new refuse reason is selected.
...
The name entered in the :guilabel:`Email Template` column populates the :guilabel:`Template Name`
field by default, but can be modified if desired. Enter the email :guilabel:`Subject` in the
corresponding field, and add the email content in the *Body* tab. If any attachments are desired,
click the *Options* tab, and add the attachment using the :icon:`fa-paperclip`
:guilabel:`Attachments` button. If an :guilabel:`Owner` should be assigned to the template, select
them using the drop-down menu. Last, enter a :guilabel:`Template Description` in the corresponding
field.

Click :guilabel:`Save` to save the new template. Upon clicking that, Odoo returns to the *Refuse
Reasons* list.
...
The applicant's email address automatically populates the :guilabel:`Applicant` field. If an email
should **not** be sent to the applicant, click the :guilabel:`Send Email` toggle to deactivate it,
and the email body is hidden.

The :guilabel:`Subject` and email body are populated according to the corresponding email template.
...
.. tip::
   ...
   In this situation, a :guilabel:`Refuse Duplicate Applications` toggle appears on the *Refuse
   Reason* pop-up window. Enable the toggle, and a :guilabel:`Duplicate Applications` field appears
   beneath it, listing the applicant's name for each duplicate application in the system.

   To refuse *all* applications for that applicant, click the :guilabel:`Refuse` button.

   The applicant is only sent one email, and all their applications for the job position are marked
   as *Refused*.
...
applicants who have been :ref:`refused by job position <recruitment/refuse_applicant/job-refused>`,
or view :ref:`all refused applicants in the database <recruitment/refuse_applicant/all-refused>`.
...
in the search bar, then click :guilabel:`Refused` in the :icon:`fa-funnel` :guilabel:`Filters`
column.
