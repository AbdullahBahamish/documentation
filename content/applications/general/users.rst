...
:guilabel:`My Preferences` to open the user's profile. Click the :guilabel:`Security` tab to view a
...
An :guilabel:`Access Control` pop-up window loads, asking to `Please confirm your identity by
entering your password`. Enter the user's password, then click :guilabel:`Confirm Password`.

The device disappears from the :guilabel:`Security` tab and can no longer be used to log into the
database.
...
.. tip::
   ...
   app, navigate to the :guilabel:`Permissions` section, and enter the desired password length in
...
configure multi-company access, navigate to the desired user by going to :menuselection:`Settings
app --> Users section --> Manage users`. Then select the desired user to open their user form.

In the :guilabel:`Access Rights` tab, under :guilabel:`Roles`, configure the :guilabel:`Companies`
and :guilabel:`Default Company` fields:
